...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const payload: StoredOS = { captures, queue, updatedAt: new Date().toISOString() };
        ...
        <section className="flex-1">
          ...
          <div className="mx-auto max-w-6xl px-4 py-6 lg:px-6">
            ...
            <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <Card><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--faint)]">Captures</p><p className="mt-2 text-3xl font-semibold">{captures.length}</p><p className="text-sm text-[var(--muted-local)]">Saved locally</p></Card>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.85fr)]">
              ...
              <aside className="space-y-6">
                ...
                <Card>
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#D94F8C]">Capture Log</p>
                  <div className="mt-3 space-y-3">
                    {captures.slice(0, 3).map((item) => (
                      <div key={item.id} className="rounded-lg border border-[var(--line-local)] bg-[var(--surface-strong)] p-3">
                        <p className="text-sm leading-6 text-[var(--muted-local)]">{item.text}</p>
                        <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--faint)]">{new Date(item.createdAt).toLocaleDateString()} - saved to review</p>
                      </div>
                    ))}
                  </div>
                </Card>
